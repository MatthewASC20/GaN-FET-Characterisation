"""Preconditions in front of the manual DC voltage tune.
...
from gan_fet.ui.operations.rig_ops import VoltageTuneAction, voltage_tune_precondition
...
def _decide(**overrides):
    ...
    return voltage_tune_precondition(**request)


def test_a_clear_rig_launches():
    assert _decide().action is VoltageTuneAction.LAUNCH
...
def test_a_latched_trip_is_refused_before_the_target_is_considered():
    ...
    assert decision.action is VoltageTuneAction.REFUSE
...
def test_offline_hardware_is_handed_back_to_the_window():
    ...
    assert _decide(hardware_offline=True).action is VoltageTuneAction.HARDWARE_OFFLINE
...
def test_a_target_exactly_at_the_safety_limit_is_allowed():
    assert _decide(target_peak_v=MAX_PEAK_V).action is VoltageTuneAction.LAUNCH
...
def test_a_stale_wavegen_offers_to_apply_first():
    ...
    assert decision.action is VoltageTuneAction.APPLY_WAVEGEN_FIRST
